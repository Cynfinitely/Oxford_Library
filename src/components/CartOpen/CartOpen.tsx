  ...
  return (
    <div>
      {CartBooks.cartBooks.length === 0 ? (
        <div>
          {' '}
          <button className="mybtn2 m-2 opacity-50" disabled onClick={handleCart}>
            Go To Checkout
          </button>
        </div>
      ) : (
        <div>
          {' '}
          <button className="mybtn2 m-2" onClick={handleCart}>
            Go To Checkout
          </button>
        </div>
      )}
                  ...
                  <Dialog.Panel className="pointer-events-auto w-screen max-w-md">
                    <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                          ...
                          <div className="flow-root">
                            <ul role="list" className="-my-6 divide-y divide-gray-200">
                              {CartBooks.cartBooks.map((book: Partial<Book>) => (
                                <li key={book.ISBN} className="flex py-6">
                      ...
                      <div className="border-t border-gray-200 py-6 px-4 sm:px-6">
                        <div className="flex justify-between text-base font-medium text-gray-900">
                          <p>Subtotal</p>
                          <p>{CartBooks.cartTotalQuantity}</p>
                        </div>
                        <p className="mt-0.5 text-sm text-gray-500">
                          Please return items before return Date.
                        </p>
                        <div className="mt-6">
                          <a
                            href="#"
